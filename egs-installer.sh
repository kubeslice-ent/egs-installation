...
kubeaccess_precheck() {
    ...
    # Print the initial variables with icons (redirect to stderr to avoid capturing in output)
    ...
    echo "  🗂️  component_kubeconfig=$component_kubeconfig" >&2
    ...
    if [ "$use_global_config" = "true" ]; then
        if [ -n "$global_kubeconfig" ] && [ -n "$global_kubecontext" ]; then
            if context_exists_in_kubeconfig "$global_kubeconfig" "$global_kubecontext"; then
                ...
                echo "Global config is used for deployment of $component_name." >&2
        ...
        if context_exists_in_kubeconfig "$global_kubeconfig" "$component_kubecontext"; then
            ...
            echo "Global kubeconfig is used with component kubecontext for deployment of $component_name." >&2
            api_server_url=$(get_api_server_url "$kubeaccess_kubeconfig" "$kubeaccess_context")
            echo "🌐 API Server URL for context '$kubeaccess_context': $api_server_url" >&2
        else
    ...
    # Only output the final kubeconfig and context, nothing else
...
run_k8s_commands_from_yaml() {
    local yaml_file=$1
# Prepare global context argument if the global context is set
if [[ -n "$global_kubecontext" && "$global_kubecontext" != "null" ]]; then
    global_context_arg="--context $global_kubecontext"
fi
...
fetch_k8s_cluster_endpoint() {
    ...
    if [ -z "$endpoint" ]; then
        ...
        if [ -z "$endpoint" ]; then
            ...
            endpoint=$(kubectl --kubeconfig="$kubeconfig" --context="$kubecontext" config view -o jsonpath='{.contexts[?(@.name == "'$kubecontext'")].context.cluster}')
        ...
        if [ -n "$inline_values" ] && [ "$inline_values" != "{}" ]; then
            ...
            if [ -z "$inline_endpoint" ] || [ "$inline_endpoint" == "null" ]; then
                ...
                if [ -z "$worker_endpoint" ]; then
                    ...
                    worker_endpoint=$(kubectl config view --kubeconfig=$kubeconfig_path $context_arg -o jsonpath='{.clusters[?(@.name == "'$(kubectl config view --kubeconfig=$kubeconfig_path $context_arg -o jsonpath='{.context.cluster}')'")].cluster.server}') # Replace this with a suitable default if needed
                else
