...
display_summary() {
    ...
    if [ "$ENABLE_INSTALL_UI" = "true" ] && [ "$KUBESLICE_UI_SKIP_INSTALLATION" = "false" ]; then
        ...
        if [ "$KUBESLICE_UI_INGRESS_ENABLED" = "true" ]; then
            ...
            node_ip=$(kubectl get nodes --kubeconfig "$kubeconfig_path" --context "$kubecontext" -o jsonpath='{.items[0].status.addresses[?(@.type=="ExternalIP")].address}' 2>/dev/null || echo "")
            if [ -z "$node_ip" ]; then
                node_ip=$(kubectl get nodes --kubeconfig "$kubeconfig_path" --context "$kubecontext" -o jsonpath='{.items[0].status.addresses[?(@.type=="InternalIP")].address}' 2>/dev/null || echo "")
            fi
