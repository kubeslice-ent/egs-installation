  ...
  if [ "$generate_summary_flag" == "true" ]; then
    ...
    for key in "${!summary[@]}"; do
      ...
      if [[ "$status" == "Success" ]]; then
        ...
        failures["$function_type|$parameter_function"]="Notfound|$status| ⚠️"
      fi
    done
...
grep_k8s_resources_with_crds_and_webhooks() {
  ...
  local fetch_resource_names="${9:-}"       # Names to filter resources
  local api_resources="${10:-all}"          # Use global API_RESOURCES variable or default to 'all'
  local webhooks="${11:-all}"
  local fetch_webhook_names="${12:-}"       # Use global WEBHOOKS variable or default to 'all'
  ...
  if [[ "$webhooks" == "all" ]]; then
    echo "🌍 Fetching all webhooks from the cluster..."
  ...
  for webhook_type in "${webhook_name_array[@]}"; do
    ...
    if [[ -n "$fetch_webhook_names" ]]; then
      ...
      local webhook_matches
      webhook_matches=$(run_command kubectl $kubeconfig --context=$kubecontext get $webhook_type --all-namespaces -o custom-columns=NAME:.metadata.name 2>/dev/null)

      if [[ -n "$webhook_matches" ]]; then
        echo "✅ Found $webhook_type:"
        echo "$webhook_matches"
        log_summary "$webhook_type Check" "$webhook_type Found:Success"
        webhook_summary+="✅ Found:\n$webhook_matches\n"
      else
        echo "❌ No $webhook_type found."
        log_summary "$webhook_type Check" "$webhook_type Not Found:Failure"
        webhook_summary+="❌ Not Found\n"
        SUCCESS=false
      fi
    fi
